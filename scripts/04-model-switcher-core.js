...
const AGENT_LOOP_SYSTEM_PROMPT = `You have an agent loop with hard tool-call limits. After the declared max steps at a time, you must either call the thinking tool before any further non-thinking tool call or stop and answer/ask the user with what you know. Use exactly this shape to reset the consecutive step counter: <fauna_tool_call>{"tool":"thinking","summary":"What I know and what I will do next."}</fauna_tool_call>. Thinking does not access external data and does not increase what you know; it only records a brief plan and resets the consecutive tool-step counter. The total max steps per run is still a hard stop and cannot be reset. After useful tool results, answer the user normally with a concise summary of what changed, what was found, or what remains; do not replay tool logs or repeat the same tool call.`;
